...
class SeedInitialHelpArticles extends Migration
{
    public function up()
    {
        ...
        foreach ($articles as $a) {
            $exists = DB::table('help_articles')->where('slug', $a['slug'])->exists();
            if ($exists) {
                continue;
            }
            DB::table('help_articles')->insert(array_merge($a, [
                'page_keys' => json_encode($a['page_keys'] ?? []),
                'is_published' => 1,
                'created_at' => $now,
                'updated_at' => $now,
            ]));
    ...
    public function down()
    {
        ...
        $slugs = collect($this->articles())->pluck('slug')->all();
        DB::table('help_articles')->whereIn('slug', $slugs)->delete();
    }
...
<ul>
  <li><strong>Ring up a customer</strong> &rarr; <a href="/sale-pos/create" target="_blank">POS &rarr; Create</a></li>
...
<h3>Help is always one click away</h3>
<p>The <i class="fa fa-question-circle"></i> button at the top of every page opens a side drawer with help for the page you're on. The search box next to it searches the whole handbook.</p>
<p class="help-tip"><strong>Tip:</strong> press <code>Ctrl + /</code> (or <code>Cmd + /</code>) on any page to jump straight to the help search.</p>

<h3>Found something missing or wrong?</h3>
<p>Email <a href="mailto:sarah@nivessa.com">sarah@nivessa.com</a> — every search that comes up empty also gets logged automatically and surfaces in the admin help-search report so we know what to add next.</p>
HTML,
            ],
            [
