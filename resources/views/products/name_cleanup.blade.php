...
<div class="mgn-wrap">
    ...
    <div class="mgn-card" style="border-color:#2E7D32;">
        ...
        <div style="margin-top:14px;padding:14px;border:1px solid #CDE3CD;background:#F3F9F3;border-radius:10px;">
            <div style="font-weight:600;color:#1B5E20;">Fill blank genres (matches YOUR categories, never invents new ones)</div>
            <p class="sub" style="margin:6px 0 10px;">For every blank-genre music product that has a Discogs release id: first checks whether you've already categorized <b>another release by the same artist</b> in the same format — if so, copies that genre, no Discogs needed. Otherwise falls back to Discogs' genre/style tags. Either way it only fills a genre that already exists as one of your sub-categories (Rock, Alt Rock, Jazz, etc.) — no new categories get created, nothing is guessed. (Discogs' own genre list isn't in relevance order, it's alphabetical, so a raw "first genre" pick is often wrong on its own.) <b>Sealed vinyl first.</b> Rate-limited (~55/min) only for the Discogs fallback; runs in batches, leave the tab open. Fully undoable. Products with no release id, or no match anywhere, won't be touched.</p>
            <div class="mgn-actions" style="margin-top:0;">
