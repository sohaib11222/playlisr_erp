@extends('layouts.app')
@section('title', 'Install safe-drop column')
...
<section class="content-header">
    <h1>Install safe-drop column</h1>
    <p>Adds one nullable column — <code>cash_registers.safe_drop_amount</code> — so the close-register flow can persist how much cash the cashier moved to the safe, and the register report can sum it. Adds only this single column; other pending migrations are not touched. Safe to run more than once.</p>
</section>
    ...
    <div class="ic-wrap">
        ...
        <div class="ic-card">
            ...
            <table class="ic-table">
                ...
                <tbody>
                    <tr>
                        <td><code>cash_registers.safe_drop_amount</code></td>
                        <td>
                            @if($has_safe_drop_amount)
            ...
            <div style="margin-top:18px;">
                @if($has_safe_drop_amount)
                    <p class="ic-done">✓ Column exists. Safe-drop persistence is fully active — no further action needed here.</p>
                @else
                    <form method="POST" action="{{ url('/admin/install-safe-drop-column/run') }}" style="margin:0;">
                        {{ csrf_field() }}
                        <button type="submit" class="ic-btn"
                                onclick="return confirm('Add cash_registers.safe_drop_amount? This is reversible — the column can be dropped later.');">
                            Install safe-drop column
                        </button>
                    </form>
                @endif
            </div>
