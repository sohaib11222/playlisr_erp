@extends('layouts.app')
@section('title', 'Install cash-deposits table')
...
<section class="content-header">
    <h1>Install cash-deposits table</h1>
    <p>Adds one table — <code>cash_deposits</code> — so each safe drop gets its own running deposit number per store (Deposit #1, #2, …) that the cashier writes on the post-it, plus a who/when/how-much audit trail. Adds only this table; other pending migrations are not touched. Safe to run more than once.</p>
</section>
    ...
    <div class="ic-wrap">
        ...
        <div class="ic-card">
            ...
            <table class="ic-table">
                ...
                <tbody>
                    <tr>
                        <td><code>cash_deposits</code></td>
                        <td>
                            @if($has_cash_deposits)
            ...
            <div style="margin-top:18px;">
                @if($has_cash_deposits)
                    <p class="ic-done">✓ Table exists. Deposit numbering is fully active — no further action needed here.</p>
                @else
                    <form method="POST" action="{{ url('/admin/install-cash-deposits-table/run') }}" style="margin:0;">
                        {{ csrf_field() }}
                        <button type="submit" class="ic-btn"
                                onclick="return confirm('Create the cash_deposits table? This is reversible — the table can be dropped later.');">
                            Install cash-deposits table
                        </button>
                    </form>
                @endif
            </div>
