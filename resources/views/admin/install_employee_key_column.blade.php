@extends('layouts.app')
@section('title', 'Install employee_key column')
...
<section class="content-header">
    <h1>Install employee_key column</h1>
    <p>Adds <code>clover_reconciliations.employee_key</code> (nullable VARCHAR) plus the composite index <code>cr_bdek</code> so the per-cashier reconcile checkbox and notes textarea on <a href="{{ url('/reports/clover-eod-reconciliation') }}">Clover EOD Reconciliation</a> can save. Idempotent — safe to re-run.</p>
</section>
    ...
    <div class="ic-wrap">
        ...
        <div class="ic-card">
            ...
            <table class="ic-table">
                ...
                <tbody>
                    <tr>
                        <td><code>clover_reconciliations.employee_key</code></td>
                        <td>
                            @if($has_employee_key)
                    ...
                    <tr>
                        <td><code>index cr_bdek (business_id, day, employee_key)</code></td>
                        <td>
                            @if($has_index)
                                <span class="ic-pill ok">already installed</span>
                            @else
            ...
            <div style="margin-top:18px;">
                @if($has_employee_key && $has_index)
                    <p class="ic-done">✓ Column and index both exist. Per-cashier reconcile + notes save should now work — try the textarea on the reconciliation page.</p>
                @else
                    <form method="POST" action="{{ url('/admin/install-employee-key-column/run') }}" style="margin:0;">
                        {{ csrf_field() }}
                        <button type="submit" class="ic-btn"
                                onclick="return confirm('Add clover_reconciliations.employee_key + index cr_bdek? Both are reversible — column is nullable, index is dropable.');">
                            Install employee_key column
                        </button>
                    </form>
                @endif
            </div>
