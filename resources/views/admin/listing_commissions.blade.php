                    ...
                    <tbody>
                        @foreach ($owedPeople as $p)
                            <tr>
                                ...
                                <td style="text-align:right; background:#EDEDED; border-left:2px solid #ccc; font-size:12px;">@if(abs($p->party_earned ?? 0) < 0.005)<span class="text-muted">—</span>@else <div style="color:#5A5045;">Owed: ${{ number_format($p->party_earned, 2) }}</div><div style="color:#2f7a4f; font-weight:600;">Paid: ${{ number_format($p->party_earned, 2) }} ✓</div>@endif</td>
