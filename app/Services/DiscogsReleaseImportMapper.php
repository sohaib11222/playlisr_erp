...
class DiscogsReleaseImportMapper
{
    ...
    public function mapFromApiPayload(int $businessId, object $payload, int $releaseId): array
    {
        ...
        $resolved = $this->resolveCategoryFromGenres($businessId, $payload->genres ?? [], $payload->styles ?? []);
     ...
     */
    private function resolveCategoryFromGenres(int $businessId, $genres, $styles): array
    {
        $warnings = [];
        $terms = [];
        ...
        foreach ($terms as $term) {
            foreach ($subs as $sub) {
                ...
                if ($subName === $term || mb_strpos($subName, $term) !== false || mb_strpos($term, $subName) !== false) {
                    return [
