...
class EbayService
{
    ...
    /**
     * @deprecated Use App\Services\Ebay\EbayListingService instead.
     */
    public function createListing($productData)
    {
        $service = new \App\Services\Ebay\EbayListingService($this->businessId, $this);
        if (!empty($productData['product_id'])) {
            return $service->listProduct((int) $productData['product_id']);
        }
        return [
            'success' => false,
            'msg' => 'createListing() is deprecated — use EbayListingService::listProduct($productId).',
        ];
    }

    /** API base URL for the configured environment (sandbox or production). */
