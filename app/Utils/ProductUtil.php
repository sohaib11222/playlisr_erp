...
class ProductUtil extends Util
{
    ...
    public function createSingleImportProductVariation($images , $product, $sku, $purchase_price, $dpp_inc_tax, $profit_percent, $selling_price, $selling_price_inc_tax, $combo_variations = [])
    {
        ...
        $source_image = file_get_contents($images);

        $path = parse_url($images, PHP_URL_PATH);
        ...
        $media_obj= new \App\Media([
            ...
            'model_media_type' => null
        ]);
        $variation->media()->saveMany($media_obj);
        }
        return true;
    }
    ...
    public function createVariableImportProductVariations($product, $input_variations, $business_id = null)
    {
        if (!is_object($product)) {
            $product = Product::find($product);
        }
        ...
        foreach ($input_variations as $key => $value) {
            ...
            if (!empty($value['variations'])) {
                ...
                $i = 0;
                foreach ($variations as $in =>  $variation) {
                     if(isset($images[$in]) && !empty($images[$in])) {
                        $source_image = file_get_contents($images[$in]);

                        $path = parse_url($images[$in], PHP_URL_PATH);
                        $new_name = time() . '_' . basename($path);
                        $dest_img = public_path() . '/uploads/' . 'media' . '/' . $new_name;
    ...
    public function filterProduct($business_id, $search_term, $location_id = null, $not_for_selling = null, $price_group_id = null, $product_types = [], $search_fields = [], $check_qty = false, $search_type = 'like'){
        ...
        $query = Product::join('variations', 'products.id', '=', 'variations.product_id')
                ...
                ->leftjoin('categories as SCAT', 'products.sub_category_id', '=', 'SCAT.id')
                ->leftjoin(
