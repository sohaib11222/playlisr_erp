...
function storeImageFromUrl($url)
{

    // Get the file contents of the image
    $imageContents = file_get_contents($url);

    // Get the file name from the URL (or you can generate your own name)
    $fileName = basename($url);

    // Define the path to store the image (this will save it in the storage/app/public/images folder)
    $path = 'public/images/' . $fileName;

    // Store the image
    \Illuminate\Support\Facades\Storage::put($path, $imageContents);

    // Optional: If you want the URL of the stored image
    $storedImageUrl = \Illuminate\Support\Facades\Storage::url($path);

    return $storedImageUrl; // You can return this URL or do other logic with it
}