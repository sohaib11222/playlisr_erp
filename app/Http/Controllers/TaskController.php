...
class TaskController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        // title/description/store/assignees; seed them as old input (this request only) so the
        // form's old() calls pick them up. Real old input (a failed submit) wins.
        if ($request->input('prefill') && !$request->session()->hasOldInput()) {
            $request->session()->now('_old_input', $request->only(['title', 'description', 'store', 'assignees']));
        }
